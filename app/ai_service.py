...
SYSTEM_PROMPT = """You are a fast, accurate MedDRA coding assistant. Given a user's lay phrase and \
a list of real MedDRA candidate terms, pick the best matches or suggest better search terms if none fit.

Only use term_ids from the candidate list. Never invent codes.
...
class AIService:
    ...
    async def refine(cls, query: str, candidates: list[dict]) -> dict | None:
        ...
        timeout = httpx.Timeout(connect=10.0, read=max(45.0, settings.AI_TIMEOUT), write=10.0, pool=10.0)
        try:
