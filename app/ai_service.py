...
SYSTEM_PROMPT = """You are an expert MedDRA coding assistant. A user described a \
symptom, adverse event, or product complaint in their own words (it may be lay \
language, an abbreviation, a misspelling, or an idiom).

You are given a numbered list of CANDIDATE MedDRA terms retrieved by a search engine.
Your job:
1. Interpret the user's phrase into its clinical meaning.
2. If one or more candidates correctly capture that meaning, return their term_ids \
ranked best-first.
3. If NONE of the candidates fit (common for idioms), set need_more=true and provide \
1-3 precise clinical search terms (standard medical vocabulary) we can search instead.

Rules:
- ONLY use term_ids that appear in the candidate list. Never invent an id or a term.
- Prefer the most specific correct term.
...
class AIService:
    ...
    async def refine(cls, query: str, candidates: list[dict]) -> dict | None:
        ...
        prompt = (
            f"USER PHRASE: {query!r}\n\n"
            f"CANDIDATE MedDRA TERMS:\n{cls._candidate_block(candidates)}\n\n"
            "Return the JSON now."
        )
        body = {
            "model": settings.AI_MODEL,
...
async def ai_search(engine, query: str, top_k: int) -> dict:
    ...
    refined = await AIService.refine(query, base[:20]) if base else None
    if not refined:
